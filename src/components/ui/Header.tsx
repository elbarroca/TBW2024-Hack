...
export default function Header() {
  ...
  const { isProfileVisible, isConnected } = useWalletState();
  ...
  const allNavLinks = isConnected 
    ? [...navLinks, { path: '/profile', label: 'Profile' }]
    : navLinks;

  // Effect to handle navigation when wallet disconnects
  useEffect(() => {
    if (!isConnected && location.pathname === '/profile') {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {/* Wallet Button */}
            <WalletMultiButton className="!bg-gradient-to-r !from-purple-600 !to-teal-400 !rounded-full !py-2" />
          </div>
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 bg-white border-t border-gray-100">
              ...
              {isConnected && (
                <div className="px-3 py-2">
                  <div className="text-sm font-medium text-gray-500 mb-2">Create Content</div>
                  {createMenuItems.map((item) => (
                    <Link
                      key={item.path}
                      to={item.path}
              ...
              <div className="px-3 py-2">
                <WalletMultiButton className="!bg-gradient-to-r !from-purple-600 !to-teal-400 !rounded-full !py-2 !w-full" />
              </div>
            </div>
          </div>
        )}
